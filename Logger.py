...
Logger.py outputs logging information to both stdout
and to log/Logger.{timestamp}.log
...
class Logger(object):
    ...
    def configure(self, **kwargs):
        ...
        if not Logger.logname: Logger.logname = kwargs.get('logfile', None)
        Logger.level = Logger.levels[kwargs.get('level', '0')]
...
"""
        ...
        module_method = '%s [%s.%s:%d]' % (duration, module_name, method_name, lnum)
    ...
    def debug( self, msg, *args, **kwargs):
        ...
        self.xml(msg)
        Logger.logger.debug( color(self._whoami()+msg), *args, **kwargs)
    def info( self, msg, *args, **kwargs):
        self.required()
        color = Logger.color['info']
        self.xml(msg)
        Logger.logger.info( color(self._whoami()+msg), *args, **kwargs)
    def warning( self, msg, *args, **kwargs):
        self.required()
        color = Logger.color['warning']
        self.xml(msg)
        Logger.logger.warning( color(self._whoami()+msg), *args, **kwargs)
    def error( self, msg, *args, **kwargs):
        self.required()
        color = Logger.color['error']
        self.xml(msg)
        Logger.logger.error( color(self._whoami()+msg), *args, **kwargs)
    def critical( self, msg, *args, **kwargs):
        self.required()
        color = Logger.color['critical']
        self.xml(msg)
        Logger.logger.critical( color(self._whoami()+msg), *args, **kwargs)
    def log( self, lvl, msg, *args, **kwargs):
        self.required()
        self.xml(msg)
        Logger.logger.log( lvl, self._whoami()+msg, *args, **kwargs)
